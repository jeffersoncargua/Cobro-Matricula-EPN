					...
					<div className={`${!enableMenu && "hidden"}  w-full md:block md:w-auto`} id="navbar-multi-level">
						<ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white">
							<li className="flex hover:text-blue-700">
								<svg xmlns="http://www.w3.org/2000/svg" aria-label="Menu" fill="currentColor" className="bi bi-house-door w-5 h-5 ms-1.5 me-1.5" viewBox="0 0 16 16">
							...
							{(user.rol === process.env.REACT_APP_ROLEMANAGE ||
								user.rol === process.env.REACT_APP_ROLECOLLAB) && (
								<>
									<li className="hover:text-blue-700">
										<button	id="dropdownNavbarNavLink" type="button" data-dropdown-toggle="dropdownNavbar" onClick={() => setEnableDrop(true)} className="flex items-center justify-between w-full py-2 px-3 text-gray-900 hover:bg-gray-100 md:hover:underline md:hover:underline-offset-8 md:hover:decoration-4 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 md:w-auto">
											<svg xmlns="http://www.w3.org/2000/svg" aria-label="GestionUser" fill="currentColor" className="bi bi-people w-5 h-5 ms-1.5 me-1.5" viewBox="0 0 16 16">
                      ...
                      <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownLargeButton">
                        ...
                        <li>
                          <NavLink to='parameters/tecnologia' onClick={()=>{setEnableDrop(false);setEnableMenu(false)}} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Tecnología</NavLink>
										...
										<svg xmlns="http://www.w3.org/2000/svg" aria-label="param" fill="currentColor" className="bi bi-calculator w-5 h-5 ms-1.5 me-1.5" viewBox="0 0 16 16">
							...
							{user.user !== null && (
								<li className="hover:text-blue-700">
									<button type="button" onClick={() => handleLogout()} className="block py-2 px-3 text-gray-900 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 ">
										Cerrar Sesión
									</button>
								</li>
							)}
