...
export const Login = ({ setEnableForm, setEnableModalRecover, setLoading }) => {
	...
	return (
		<form
			className="border border-slate-600 p-2 rounded-lg w-80 text-sm"
			onSubmit={handleSubmit(HandleLogin)}
		>
			<label
				htmlFor="email"
				className="text-start block mb-2 font-medium text-gray-900 dark:text-white"
			>
				Ingresa tu Correo
			</label>
			<div className="relative mb-1">
				...
				<input
					...
					className={`bg-gray-50 border ${errors.email ? "text-red-500  focus:outline-red-700" : "text-slate-800 focus:outline-blue-500 "} rounded-lg block w-full ps-10 p-2.5`}
			...
			<div className="flex">
				...
				<div className="relative w-full">
					<input
						...
						id="password"
						{...register("password", { required: message.req.password })}
						className={`rounded-none rounded-e-lg bg-gray-50 border ${errors.email ? "text-red-500  focus:outline-red-700" : "text-slate-800 focus:outline-blue-500 "} block flex-1 min-w-0 w-full border-gray-300 p-2.5`}
						placeholder="Mjug&/%113"
					/>
					<button
						type="button"
						className="absolute inset-y-0 end-0 flex items-center pe-2.5 "
						onClick={() => setEnablePass(!enablePass)}
					>
						{!enablePass
							? <svg
									xmlns="http://www.w3.org/2000/svg"
									...
									height="16"
									fill="currentColor"
									className="bi bi-eye-fill hover:text-gray-500"
									viewBox="0 0 16 16"
								>
							...
							: <svg
									...
									height="16"
									fill="currentColor"
									className="bi bi-eye-slash-fill hover:text-gray-500"
									viewBox="0 0 16 16"
								>
			...
			{errors.password && (
				<ErrorMessageValidator message={errors.password.message} />
			)}
			<div className="mt-2 flex flex-col ">
				{showButtonLoading
					...
					: <>
							<button
								type="submit"
								className="mx-auto px-3 py-2.5 text-center bg-cyan-500 hover:bg-cyan-600 hover:text-white rounded-lg"
							>
								Iniciar Sesión
							</button>
							...
							<button
								type="button"
								onClick={() => setEnableModalRecover(true)}
								to={"forgetPassword"}
								className="text-indigo-900 font-semibold hover:text-blue-950 hover:underline px-2.5 py-2.5 "
							>
