...
export const FormRecover = () => {
	...
	const handleChangePass = async (resetPasswordRequest) => {
		...
		if (response.isSuccess) {
			...
			SwalFailed(
				"Oopss..",
				response.message,
				"Intenta nuevamente enviando una solicitud con tus datos válidos. Para mayor información solicita ayuda al administrador del sistema",
			);
		}
	...
	return (
		...
		>
			<label
				htmlFor="password"
				className=" text-center block mb-2 font-medium text-slate-100 dark:text-white"
			>
			...
			<div className="flex">
				...
				<div className="relative w-full">
					<input
						...
						className="rounded-none rounded-e-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full border-gray-300 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
			...
			<div className="flex">
				...
				<div className="relative w-full">
					<input
						type={!enablePass2 ? "password" : "text"}
					...
					<button
						type="button"
						className="absolute inset-y-0 end-0 flex items-center pe-2.5 "
						onClick={() => setEnablePass2(!enablePass2)}
					>
						{!enablePass2
							? <svg
									xmlns="http://www.w3.org/2000/svg"
			...
			{errors.confirmPassword && (
				<ErrorMessageValidator message={errors.confirmPassword.message} />
			)}
			{showButtonLoading
				? <ButtonLoading />
				: <div className="mt-2 flex flex-col">
						<button
							type="submit"
							className="px-2.5 py-2.5 text-center bg-cyan-500 hover:bg-cyan-600 hover:text-white rounded-lg"
						>
							Confirmar
						</button>
					</div>}
