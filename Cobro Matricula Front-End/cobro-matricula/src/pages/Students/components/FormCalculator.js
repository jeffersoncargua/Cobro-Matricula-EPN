		...
		>
				...
				<section className="p-4 flex flex-col gap-y-8">
							...
							<li className="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
								<div className="flex items-center ps-3">
									<input
										...
										{...register("gratuidad", {
											required: message.req.gratuidad,
										})}
									/>
									<label
					...
					<div className="flex flex-col gap-y-4 text-start">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 md:gap-y-0">
							<label
								htmlFor="primera"
								className="text-white underline underline-offset-4 font-bold"
							>
								Primera Matrícula:{" "}
							</label>
							<input
								...
								min={0}
								defaultValue={0}
								className="ms-3 p-2 focus:outline-none focus:border-sky-500 focus:ring focus:ring-sky-500 rounded-lg text-end mb-2"
								placeholder="0"
								{...register("primera", {
									required: message.req.primera,
									pattern: { value: patterns.entero, message: message.primera },
									valueAsNumber: true,
								})}
							/>
							{errors.primera && (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 md:gap-y-0">
							...
							<input
								...
								placeholder="0"
								{...register("segunda", {
									required: message.req.segunda,
									pattern: { value: patterns.entero, message: message.segunda },
									valueAsNumber: true,
								})}
							/>
							{errors.segunda && (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 md:gap-y-0">
							...
							<input
								...
								placeholder="0"
								{...register("tercera", {
									required: message.req.tercera,
									pattern: { value: patterns.entero, message: message.tercera },
									valueAsNumber: true,
								})}
							/>
							{errors.tercera && (
			...
			<div className="w-[90%] mx-auto flex flex-row justify-between space-x-10 mt-2">
				<button
					...
					className="w-full bg-green-700 text-white p-4 rounded-lg transition delay-150 duration-300 ease-in-out hover:bg-blue-500 hover:text-black hover:scale-[1.05]"
				>
					Nueva Consulta
				</button>
				<button
					type="submit"
					className="w-full bg-blue-700 text-white p-4 rounded-lg transition delay-150 duration-300 ease-in-out hover:bg-green-500 hover:text-black hover:scale-[1.05]"
				>
					Calcular
				</button>
			</div>
